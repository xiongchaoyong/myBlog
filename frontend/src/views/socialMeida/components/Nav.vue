<template>
  <div class="nav-container">
    <div class="left-section">
      <div class="avatar-container" @click="showUserDialog = true">
        <Avatar :avatar="loginStore.user?.avatar" :username="loginStore.user?.username || '用户'" />
      </div>
      <span class="name">{{ loginStore.user?.username || '用户名' }}</span>
    </div>
    ...
    <div class="right-section">
      ...
      <router-link to="/myroom" class="menu-item">我的</router-link>
    </div>
    <!-- 用户信息弹窗 -->
      ...
      <div v-if="loginStore.user" class="user-info-flex">
        ...
        <div class="user-info-card">
          ...
          <div class="user-info-row">
            ...
            <span>{{ loginStore.user.gender === 1 ? '男' : loginStore.user.gender === 2 ? '女' : '未知' }}</span>
          </div>
          <div class="user-info-row">
            <span class="label">地区：</span>
            <span>{{ loginStore.user.region || '未设置' }}</span>
          </div>
          <div class="user-info-row">
            <span class="label">个性签名：</span>
            <span>{{ loginStore.user.signature || '这个人很懒，什么都没留下~' }}</span>
          </div>
          <div class="user-info-row">
            <span class="label">注册时间：</span>
            <span>{{ loginStore.user.created_at || '未知' }}</span>
          </div>
 ...
 <!-- 用户信息弹窗 -->
    <el-dialog v-model="showTalkDialog" title="🫥" width="600px" >
      <span style="font-size: 20px">😭这个社交平台项目是我以前写的,那时候是一个新手,很多都不懂,所以这个项目的性能不好,可能还存在一些bug。我本来准备改的,但是我看了头痛,不想改了,以后有时间再改吧😭</span>
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="showTalkDialog = false">关闭</el-button>
        </span>
      </template>
    </el-dialog>
